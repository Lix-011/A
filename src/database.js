const Database = require('better-sqlite3');
const path = require('path');

const db = new Database(path.join(__dirname, '..', 'data.sqlite'));
db.pragma('journal_mode = WAL');

db.exec(`
  CREATE TABLE IF NOT EXISTS admin_stats (
    userId TEXT PRIMARY KEY,
    messages INTEGER NOT NULL DEFAULT 0,
    presenceSeconds INTEGER NOT NULL DEFAULT 0,
    supportVcSeconds INTEGER NOT NULL DEFAULT 0,
    modActions INTEGER NOT NULL DEFAULT 0,
    events INTEGER NOT NULL DEFAULT 0
  );

  CREATE TABLE IF NOT EXISTS voice_sessions (
    userId TEXT PRIMARY KEY,
    channelId TEXT NOT NULL,
    joinedAt INTEGER NOT NULL,
    isSupportVc INTEGER NOT NULL DEFAULT 0
  );

  CREATE TABLE IF NOT EXISTS daily_tasks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    userId TEXT NOT NULL,
    taskKey TEXT NOT NULL,
    taskText TEXT NOT NULL,
    date TEXT NOT NULL,
    completed INTEGER NOT NULL DEFAULT 0,
    completedAt INTEGER,
    messageId TEXT,
    createdAt INTEGER NOT NULL
  );

  CREATE INDEX IF NOT EXISTS idx_daily_tasks_date ON daily_tasks (date);
`);

function ensureRow(userId) {
  db.prepare(`INSERT OR IGNORE INTO admin_stats (userId) VALUES (?)`).run(userId);
}

function addMessage(userId) {
  ensureRow(userId);
  db.prepare(`UPDATE admin_stats SET messages = messages + 1 WHERE userId = ?`).run(userId);
}

function addVoiceSeconds(userId, seconds, isSupportVc) {
  ensureRow(userId);
  if (isSupportVc) {
    db.prepare(`UPDATE admin_stats SET supportVcSeconds = supportVcSeconds + ? WHERE userId = ?`).run(seconds, userId);
  } else {
    db.prepare(`UPDATE admin_stats SET presenceSeconds = presenceSeconds + ? WHERE userId = ?`).run(seconds, userId);
  }
}

function addModAction(userId, count = 1) {
  ensureRow(userId);
  db.prepare(`UPDATE admin_stats SET modActions = modActions + ? WHERE userId = ?`).run(count, userId);
}

function addEvent(userId, count = 1) {
  ensureRow(userId);
  db.prepare(`UPDATE admin_stats SET events = events + ? WHERE userId = ?`).run(count, userId);
}

function getStats(userId) {
  ensureRow(userId);
  return db.prepare(`SELECT * FROM admin_stats WHERE userId = ?`).get(userId);
}

function getAllStats() {
  return db.prepare(`SELECT * FROM admin_stats`).all();
}

function resetStats(userId) {
  db.prepare(`
    UPDATE admin_stats
    SET messages = 0, presenceSeconds = 0, supportVcSeconds = 0, modActions = 0, events = 0
    WHERE userId = ?
  `).run(userId);
}

function resetAll() {
  db.prepare(`
    UPDATE admin_stats
    SET messages = 0, presenceSeconds = 0, supportVcSeconds = 0, modActions = 0, events = 0
  `).run();
}

// --- جلسات الصوت (لحساب مدة البقاء في الروم) ---

function startVoiceSession(userId, channelId, isSupportVc) {
  db.prepare(`
    INSERT INTO voice_sessions (userId, channelId, joinedAt, isSupportVc)
    VALUES (?, ?, ?, ?)
    ON CONFLICT(userId) DO UPDATE SET channelId = excluded.channelId, joinedAt = excluded.joinedAt, isSupportVc = excluded.isSupportVc
  `).run(userId, channelId, Date.now(), isSupportVc ? 1 : 0);
}

function endVoiceSession(userId) {
  const session = db.prepare(`SELECT * FROM voice_sessions WHERE userId = ?`).get(userId);
  if (session) {
    db.prepare(`DELETE FROM voice_sessions WHERE userId = ?`).run(userId);
  }
  return session;
}

// --- المهام اليومية ---

function assignDailyTask(userId, taskKey, taskText, date) {
  const info = db.prepare(`
    INSERT INTO daily_tasks (userId, taskKey, taskText, date, createdAt)
    VALUES (?, ?, ?, ?, ?)
  `).run(userId, taskKey, taskText, date, Date.now());
  return info.lastInsertRowid;
}

function setTaskMessageId(taskId, messageId) {
  db.prepare(`UPDATE daily_tasks SET messageId = ? WHERE id = ?`).run(messageId, taskId);
}

// يتأكد إن الشخص اللي ضغط الزر هو نفسه صاحب المهمة قبل ما يعلّمها منجزة
function completeTask(taskId, userId) {
  const task = db.prepare(`SELECT * FROM daily_tasks WHERE id = ?`).get(taskId);
  if (!task) return { ok: false, reason: 'not_found' };
  if (task.userId !== userId) return { ok: false, reason: 'not_owner' };
  if (task.completed) return { ok: false, reason: 'already_done' };

  db.prepare(`UPDATE daily_tasks SET completed = 1, completedAt = ? WHERE id = ?`).run(Date.now(), taskId);
  return { ok: true, task };
}

function getTasksByDate(date) {
  return db.prepare(`SELECT * FROM daily_tasks WHERE date = ? ORDER BY id`).all(date);
}

module.exports = {
  addMessage,
  addVoiceSeconds,
  addModAction,
  addEvent,
  getStats,
  getAllStats,
  resetStats,
  resetAll,
  startVoiceSession,
  endVoiceSession,
  assignDailyTask,
  setTaskMessageId,
  completeTask,
  getTasksByDate,
};
