...
// --- زر "تم الإنجاز" لمهام اليوم ---
client.on('interactionCreate', async interaction => {
  if (!interaction.isButton()) return;
  if (!interaction.customId.startsWith('task_done_')) return;

  const taskId = Number(interaction.customId.replace('task_done_', ''));
  const result = db.completeTask(taskId, interaction.user.id);

  if (!result.ok) {
    const messages = {
      not_found: 'ما لقيت هذي المهمة.',
      not_owner: 'هذي المهمة مو مهمتك، ما تقدر تعلّمها منجزة.',
      already_done: 'أنت خلصت هذي المهمة من قبل.',
    };
    await interaction.reply({
      content: messages[result.reason] || 'صار خطأ أثناء تسجيل الإنجاز.',
      ephemeral: true,
    });
    return;
  }

  await interaction.reply({ content: 'تم تسجيل إنجاز المهمة ✅', ephemeral: true });

  try {
    const originalEmbed = interaction.message.embeds[0];
    const updatedEmbed = EmbedBuilder.from(originalEmbed)
      .setColor(0x2ecc71)
      .setFooter({ text: `تم الإنجاز ✅ بواسطة ${interaction.user.username}` });

    await interaction.message.edit({ embeds: [updatedEmbed], components: [] });
  } catch (err) {
    console.error('فشل تحديث رسالة المهمة بعد الإنجاز:', err);
  }
});
