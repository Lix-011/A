  ...
  try {
    ...
    for (let i = 0; i < membersArray.length; i++) {
      ...
      const embed = new EmbedBuilder()
        .setTitle('📌 مهمة يومية')
        .setDescription(`<LaTex>{member}، مهمتك:\n**</LaTex>{task.text}**`)
        .setColor(0xFFA500);

      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`task_done_<LaTex>{taskId}`).setLabel('تم الإنجاز ✅').setStyle(ButtonStyle.Success)       );        const msg = await channel.send({ content: `</LaTex>{member}`, embeds: [embed], components: [row] });
