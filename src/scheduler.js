...
async function assignDailyTasks(guild) {
  try {
    ...
    // جلب كل الأعضاء لضمان دقة القائمة
    await guild.members.fetch();

    // اختيار كل الأعضاء الذين يملكون الرتبة (وليس عشوائياً)
    const eligibleMembers = guild.members.cache.filter(
      m => !m.user.bot && m.roles.cache.has(config.ADMIN_ROLE_ID)
    );

    if (eligibleMembers.size === 0) {
      console.error(`❌ لم أجد أي عضو برتبة الإدارة: ${config.ADMIN_ROLE_ID}`);
      return null;
    }
    ...
    // تحويل الأعضاء إلى مصفوفة
    const membersArray = [...eligibleMembers.values()];
    
    // بناء مصفوفة مهام كافية لجميع الأعضاء
    let taskPool = [];
    ...
    for (let i = 0; i < membersArray.length; i++) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`${member}، مهمتك اليوم هي:\n\n**<LaTex>{task.text}**`)         .setColor(0xFFA500)         .setTimestamp();        const row = new ActionRowBuilder().addComponents(         new ButtonBuilder()           .setCustomId(`task_done_</LaTex>{taskId}`)
          .setLabel('تم الإنجاز ✅')
